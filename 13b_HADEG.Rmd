...
# Save the HADEG_abundance
# write.csv(long_data, "long_data_cpm.csv")

# Merge metadata and abundance
HADEG_abundance <-  metadata %>% dplyr::left_join(long_data, by=c("Sample"), multiple = "all", relationship = "many-to-many")

# Save the HADEG_abundance
write.csv(HADEG_abundance, "HADEG_abundance.csv")
...
HADEG_abundance_tax <-  HADEG_abundance %>% dplyr::left_join(Taxa, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many")

# Save the HADEG_abundance
write.csv(HADEG_abundance_tax, "HADEG_abundance_tax.csv")


# Load HADEG_genes_converted
read_ods("HADEG_blast.ods", sheet = "HADEG",  col_names = TRUE) ->
HADEG_gene

# Merge metadata, abundance and annotation
HADEG_abundance_tax_blast <-  HADEG_abundance_tax %>% dplyr::left_join(HADEG_gene, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many") %>% drop_na(Gene) %>% subset(tax_kingdom %in% c("Archaea", "Bacteria", "Algae", "Fungi"))

# Save the HADEG_abundance
write.csv(HADEG_abundance_tax_blast, "HADEG_abundance_tax_blast.csv")


# Load the pathways
read_ods("HADEG_blast.ods", sheet = "Pathways",  col_names = TRUE) ->
Pathways

# Merge metadata, abundance and annotation
HADEG_abundance_tax_blast_path <-  HADEG_abundance_tax_blast %>% dplyr::left_join(Pathways, by=c("qseqid"), multiple = "all", relationship = "many-to-many") %>% drop_na(Gene) %>% subset(tax_kingdom %in% c("Archaea", "Bacteria", "Algae", "Fungi"))

# Save the HADEG_abundance
write.csv(HADEG_abundance_tax_blast_path, "HADEG_abundance_tax_blastpathway.csv") #

```


**Plot abundance of Hydrocarbon Aerobic Degradation genes in OSPW and RO**
```{r bar plot data, cache = T}
# Alkanes 
Ab_HADEG <- HADEG_abundance_tax_blast_path %>%
  filter(Hydro_group == "Alkanes") %>%
...
Ab_HADEG %>%
  ...
  geom_point(aes(x = cpm, y = Gene), colour = "#8B4500", size = 12, position = position_dodge(width = 1)) +
  ylab("Genes") +  
...
ggsave("Figures/Genes_HADEG_OSPW_RO_alkanes.svg", width = 65, height = 40, units = "cm")
...
ggsave("Figures/Genes_HADEG_OSPW_RO_alkenes.svg", width = 65, height = 40, units = "cm")
...
ggsave("Figures/Genes_HADEG_OSPW_RO_aromatics.svg", width = 65, height = 105, units = "cm")
...
# Biosurfactants
Ab_HADEG <- HADEG_abundance_tax_blast_path %>%
  filter(Hydro_group == "Biosurfactants") %>%
  # You can uncomment the filter if you want OSPW only
  mutate(Time = factor(Time, levels = c("D-0", "D6", "D19", "D40", "D60"))) %>%
...
ggsave("Figures/Genes_HADEG_OSPW_RO_Biosurfactants.svg", width = 65, height = 40, units = "cm")


# Plastics
Ab_HADEG <- HADEG_abundance_tax_blast_path %>%
  filter(Hydro_group == "Plastics") %>%
  # You can uncomment the filter if you want OSPW only
  mutate(Time = factor(Time, levels = c("D-0", "D6", "D19", "D40", "D60"))) %>%
...
ggsave("Figures/Genes_HADEG_OSPW_RO_Plastics.svg", width = 65, height = 105, units = "cm")
```
...
# Step 2: Sum again per Phylum to get the total sum for each Phylum in each Water_type and Time
  group_by(Hydro_group, Water_type, Time, Phylum) %>%
  summarise(cpm = sum(cpm)/100, .groups = "drop") 

  # Summarize the counts of genes for each Phylum at each level
global_order <- Ab_HADEG %>%
  group_by(Phylum) %>%
...
#  scale_y_continuous(limits = c(0, 100)) +  # Set limits from 0 to 100 for percentages
  ...
  facet_wrap(~ Water_type + Hydro_group, ncol = 5) +
  # Apply custom colors using scale_fill_manual
  scale_fill_manual(values = phylum_colors)

ggsave("Figures/Taxa_origins_HADEG_degraders_Phylum_OSPW_RO.svg", width = 65, height = 35, units = "cm")

Ab_HADEG2 %>%
#filter(Hydro_group == c("Alkanes", "Alkenes", "Aromatics")) %>%
mutate(Water_type = factor(Water_type, levels = c("RO", "OSPW"))) %>%
mutate(Time = factor(Time, levels = c("D-0", "D6", "D19", "D40", "D60"))) %>%
mutate(Hydro_group = factor(Hydro_group, levels = c("Alkanes", "Alkenes", "Aromatics", "Biosurfactants", "Plastics"))) %>%
group_by(Water_type, Hydro_group, Time) %>%
arrange(.by_group = TRUE, desc(cpm)) %>%
kable(., digits = c(0, 1, 1, 1, 0, 1)) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F)
...
order_colors <- c(Burkholderiales = "#dbdb8d", Rhizobiales = "#2ca02c", Caulobacterales = "#ff7f0e", `Unclassified Pseudomonadota` = "#8B6914", Helotiales = "#8c564b", `Unclassified bacteria` = "#7FFF00", Sphingomonadales = "#A52A2A", `Beta Proteobacteria` = "#FF8C00", Flavobacteriales = "#9467bd", Myxococcales = "#EED5B7", Micromonosporales = "#17becf", Peronosporales = "#98df8a", Cytophagales  = "#1f77b4", `Unclassified Bacteroidota` = "#ee82ee", Gemmatimonadales = "#D2691E", Gemmatales = "#FF1493", Pleosporales = "#8B0A50", `Unclassified archaea` = "#aec7e8", Pythiales = "#c49c94", `Alpha Proteobacteria` = "#9edae5", Holophagales = "#d62728", Rhodocyclales = "#bcbd22", Chaetothyriales = "#e377c2", Nostocales = "#0000FF", Sebacinales  = "#458B00", Oomycota = "#EEC900", Leotiomycetes = "#00008B", Saprolegniales = "#8B3A3A", Acidobacteriotales = "#8B6969", Alteromonadales = "#1E90FF", Chytridiales = "#c7c7c7", Actinomycetia = "#CD6600", Rhodospirillales = "#2F4F4F", Nitrosomonadales = "#104E8B", Rhodobacterales = "#008B00", Nevskiales  = "#8B5742", Pseudomonadales = "#FFA500", Verrucomicrobiales = "#8B2500", `Unclassified Armatimonadota` = "#FF83FA", Xylariales = "#96CDCD", Bacteriovoracales = "#551A8B", `Delta Proteobacteria` = "#FF0000", Thermales = "#436EEE", `Unclassified Acidobacteriota` = "#00CD66", `Unclassified Planctomycetota` = "#4682B4", Auriculariales = "#D02090", Chitinophagales = "#CD4F39", Sphingobacteriales = "#00868B", Bacillales = "#8B2252", `Unclassified Cyanobacteriota` = "#EEEE00", Xanthomonadales  = "#8B7E66")



Ab_HADEG_order <- HADEG_abundance_tax_blast_path %>%
  # You can uncomment the filter if you want OSPW only
  mutate(Time = factor(Time, levels = c("D-0", "D6", "D19", "D40", "D60"))) %>%
...
# Step 2: Sum again per tax_order to get the total sum for each tax_order in each Water_type and Time
  group_by(Hydro_group, Water_type, Time, tax_order) %>%
  summarise(cpm = sum(cpm)/100, .groups = "drop") 

  # Summarize the counts of genes for each tax_order at each level
global_order <- Ab_HADEG_order %>%
  group_by(tax_order) %>%
  summarise(sum_cpm = sum(cpm), .groups = "drop") %>%
...
ggsave("Figures/Taxa_origins_HADEG_degraders_Order_OSPW_RO.svg", width = 45, height = 35, units = "cm")
