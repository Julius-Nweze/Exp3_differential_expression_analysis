...
# Save the NA_abundance
# write.csv(long_data, "long_data_cpm.csv")

# Merge metadata and abundance
NA_abundance <-  metadata %>% dplyr::left_join(long_data, by=c("Sample"), multiple = "all", relationship = "many-to-many")

# Save the NA_abundance
write.csv(NA_abundance, "NA_abundance.csv")
...
NA_abundance_tax <-  NA_abundance %>% dplyr::left_join(Taxa, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many")

# Save the NA_abundance
write.csv(NA_abundance_tax, "NA_abundance_tax.csv")


# Load NA_genes_converted
read_ods("NA_genes_converted.ods", sheet = "NA_genes",  col_names = TRUE) ->
Na_gene

# Merge metadata, abundance and annotation
NA_abundance_tax_blast <-  NA_abundance_tax %>% dplyr::left_join(Na_gene, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many") %>% drop_na(Gene) %>% subset(tax_kingdom %in% c("Archaea", "Bacteria", "Algae", "Fungi"))

# Save the NA_abundance
write.csv(NA_abundance_tax, "NA_abundance_tax_blast.csv")
...
Ab_NA %>%
  ...
  geom_point(aes(x = cpm, y = Gene_codes), colour = "#8B4500", size = 12, position = position_dodge(width = 1)) +
  ylab("Genes") +  
...
ggsave("Figures/Genes_NA_OSPW_RO.svg", width = 65, height = 40, units = "cm")
...
ggsave("Figures/Taxa_origins_NA_degraders_Phylum_OSPW_RO.svg", width = 45, height = 35, units = "cm")
...
ggsave("Figures/Taxa_origins_NA_degraders_Order_OSPW_RO.svg", width = 45, height = 35, units = "cm")
