...
# Save the Sulf_abundance
# write.csv(long_data, "long_data_cpm.csv")

# Merge metadata and abundance
Sulf_abundance <-  metadata %>% dplyr::left_join(long_data, by=c("Sample"), multiple = "all", relationship = "many-to-many")

# Save the Sulf_abundance
write.csv(Sulf_abundance, "Sulf_abundance.csv")
...
Sulf_abundance_tax <-  Sulf_abundance %>% dplyr::left_join(Taxa, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many")

# Save the Sulf_abundance
write.csv(Sulf_abundance_tax, "Sulf_abundance_tax.csv")


# Load the gene
read_ods("Sulf_genes_converted.ods", sheet = "Gene",  col_names = TRUE) ->
gene

# Load the gene
read_ods("Sulf_genes_converted.ods", sheet = "Pathway",  col_names = TRUE) ->
Path

# Load Sulf_genes_converted
read_ods("Sulf_genes_converted.ods", sheet = "Sulf_genes",  col_names = TRUE) ->
Sulf_gene
...
Sulf_gene_path <-  Sulf_gene %>% dplyr::left_join(Path_genes, by=c("qseqid"), multiple = "all", relationship = "many-to-many") #%>% drop_na(Gene)

# Save the Sulf_gene_path
write.csv(Sulf_gene_path, "Sulf_gene_path.csv")

# Merge metadata, abundance and annotation
Sulf_abundance_tax_blast <-  Sulf_abundance_tax %>% dplyr::left_join(Sulf_gene_path, by=c("Gene_ID"), multiple = "all", relationship = "many-to-many") %>% drop_na(Pathways) %>% subset(tax_kingdom %in% c("Archaea", "Bacteria", "Algae", "Fungi"))

# Save the Sulf_abundance
write.csv(Sulf_abundance_tax_blast, "Sulf_abundance_tax_blast.csv")
```
...
Ab_NA <- Sulf_abundance_tax_blast %>%
...
Ab_NA <-  Ab_NA %>% dplyr::left_join((unique(Path_genes %>% dplyr::select(Gene, Pathways))), by=c("Gene"), multiple = "all", relationship = "many-to-many") %>% drop_na(Water_type)

path_colour = c("Assimilatory sulfate reduction" = "#458B00", "Dissimilatory sulfur reduction and oxidation" = "#8B4513",  "Sulfur reduction" = "#00008B", "Sulfur oxidation" = "#FF4040", "SOX systems" = "#FF8C00", "Link b/4 in/organic sulfur transformation" = "#00FFFF", "Others" = "#2F4F4F")

Ab_NA %>%
  mutate(Pathways = factor(Pathways, levels = c("Assimilatory sulfate reduction", "Dissimilatory sulfur reduction and oxidation",  "Sulfur reduction", "Sulfur oxidation", "SOX systems", "Link b/4 in/organic sulfur transformation", "Others"))) %>%
  mutate(Gene = factor(Gene, levels = c("cysC","cysD","cysH","cysI","cysJ","cysN","cysNC","cysQ","nrnA","sat1","sir","aprA","aprB","dsrA","dsrB","dsrC","dsrD","dsrE","dsrF","dsrH","dsrJ","dsrK","dsrL","dsrM","dsrN","dsrO","dsrP","dsrT","qmoA","qmoB","qmoC","rdsr", "sat", "asrA","asrB","asrC","fsr","hydA","hydB","hydD","hydG","mccA","otr","psrA","psrB","psrC","rdlA","shyA","shyB","shyC","shyD","sreA","sreB","sreC","sudA","sudB","ttrA","ttrB","ttrC","soxA","soxB","soxC","soxD","soxX","soxY","soxZ","doxA","doxD","fccA","fccB","glpE","soeA","soeB","soeC","sorA","sorB","sqr","sseA","tsdA","tsdB","phsA","phsB","phsC","tetH","sor","acuI","acuK","acuN","betA","betB","betC","comA","comB","comC","comD","comE","dddA","dddC","dddD","dddK","dddL","dddP","dddQ","dddT","dddW","dddY","ddhA","ddhB","ddhC","dmdA","dmdB","dmdC","dmdD","dmoA","dmsA","dmsB","dmsC","dsyB","gdh","hpsN","hpsO","hpsP","iseJ","isfD","mddA","mdh","mtsA","mtsB","prpE","pta","sfnG","slcC","slcD","sqdB","sqdD","sqdX","tauX","tauY","tmm","toa","tpa","yihQ","cuyA","cysE","cysK","cysM","cysO","hdrA1","hdrA2","hdrB1","hdrB2","hdrC1","hdrC2","hdrD","hdrE","mccB","metA","metB","metC","metX","metY","metZ","msmA","msmB","mtoX","ssuD","ssuE","suyA","suyB","tauD","tbuB","tbuC","tmoC","tmoF","touC","touF","xsc","cuyZ","cysA","cysP","cysU","cysW","cysZ","hpsK","hpsL","hpsM","iseK","iseL","iseM","sbp","sgpA","sgpB","sgpC","soxL","ssuA","ssuB","ssuC","sulP","tauA","tauB","tauC","tauE","tauZ","tusA","tusB","tusC","tusD","tusE"))) %>%
  ...
  geom_point(aes(x = cpm, y = Gene,  colour = Pathways), size = 12, position = position_dodge(width = 1)) +
  ylab("Genes") +  
...
ggsave("Figures/Genes_Sulf_OSPW_RO.svg", width = 85, height = 55, units = "cm")
...
# Step 3: Sum again per Phylum to get the total sum for each Phylum in each Water_type and Time
  ...
  summarise(cpm = sum(CPM)/100000, .groups = "drop") 
...
Ab_NA2 %>% 
 ...
 mutate(Time = factor(Time, levels = c("D-0", "D6", "D19", "D40", "D60"))) %>%
  ...
  xlab("Time") +
  ylab("Relative abundance (count per million * 10^5)") +
...
ggsave("Figures/Taxa_origins_Sulf_degraders_Phylum_OSPW_RO.svg", width = 45, height = 35, units = "cm")
...
ggsave("Figures/Taxa_origins_Sulf_degraders_Order_OSPW_RO.svg", width = 45, height = 35, units = "cm")
