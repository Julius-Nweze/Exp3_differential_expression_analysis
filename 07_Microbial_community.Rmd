...
read_ods("feature_table_normalized_microbes.ods", sheet = "Microbes",  col_names = TRUE) ->
Abs
...
# Merge the two data
Abs_metadata <- metadata %>% dplyr::right_join(Abs, by=c("SampleID"), multiple = "all", relationship = "many-to-many") 

# Save in .csv file
write_csv(Abs_metadata, "Root_microbial_abundance.csv")
...
# For prokaryotes
DEGs_taxa_microbes <- Abs_metadata %>%
#  subset(Domain %in% c("Archaea", "Bacteria")) %>%
...
DEGs_taxa_microbes2 %>% 
 mutate(Time = factor(Time, levels = c("D-0", "D19", "D40", "D6", "D60"))) %>%
...
ggsave("Figures/Phylum_abundance_Exp3_contig.svg", width = 50, height = 40, units = "cm")
